...
async def count_days_func(message: Message):
    ...
    await bot.set_state(user_id=message.from_user.id, state=HabitState.for_count_days)
...
async def get_count_days(message: Message):
    ...
    if not message.text.isdigit() or not 18 <= int(message.text) <= 254:
        await bot.send_message(chat_id=message.from_user.id, text="Нужно ввести целое число от 18 до 254")
    else:
